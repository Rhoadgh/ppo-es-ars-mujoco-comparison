...
class Worker(object):
    def __init__(self, env_seed, env_name='', policy_params=None, deltas=None, rollout_length=1000, delta_std=0.02, record_video=False, logdir='data'):
        ...
        # # --- SURVIVAL BONUS TOGGLE ---
        # # Comment the following lines out if you want to keep the original reward
        # if any(name in env_name for name in ["Walker2d", "Hopper", "Ant"]):
        #     self.env = NoSurvivalBonusWrapper(self.env)
        # # -----------------------------
...
class NoSurvivalBonusWrapper(gym.RewardWrapper):
    """
    Subtracts the survival bonus from MuJoCo environments.
    In Walker2d and Hopper, this is typically 1.0 per timestep.
    """
...
class ARSLearner(object):
    ...
    def aggregate_rollouts(self, num_rollouts=None, evaluate=False):
        ...
        # 1. Initialize lists to prevent UnboundLocalError
        rollout_rewards = []
        deltas_idx = []

        # 2. Launch rollouts on workers
        rollout_ids = [worker.do_rollouts.remote(policy_id, num_rollouts=num_rollouts_per_worker,
                                                 shift=self.shift, evaluate=evaluate)
                       for worker in self.workers]

        # Handle remainder if num_deltas doesn't divide evenly
        if num_deltas % self.num_workers > 0:
            rollout_ids += [worker.do_rollouts.remote(policy_id, num_rollouts=1,
                                                      shift=self.shift, evaluate=evaluate)
                            for worker in self.workers[:(num_deltas % self.num_workers)]]

        # 3. Collect results from Ray
        ...
        # 4. If evaluating, just return the raw rewards
        if evaluate:
            return rollout_rewards

        # --- ARS V2-t GRADIENT LOGIC START ---

        # 5. Identify the max reward for each pair (pos, neg)
        # This helps us rank which random directions were most "promising"
        max_rewards = np.max(rollout_rewards, axis=1)

        # 6. Sort and pick indices of the top 'b' (deltas_used) directions
        # argsort gives ascending; [-self.deltas_used:] takes the largest
        idx = np.argsort(max_rewards)[-self.deltas_used:]

        # 7. Filter for only the top-performing directions
        best_rewards = rollout_rewards[idx, :]
        best_deltas_idx = np.array(deltas_idx)[idx]

        # 8. Calculate Sigma_R (Standard deviation of all rewards in the update)
        # This acts as an adaptive learning rate scaler (Section 3 of the paper)
        sigma_r = np.std(best_rewards) + 1e-8

        # 9. Calculate the weighted sum of directions
        # Gradient is estimated by the difference between positive and negative perturbations
        g_hat, _ = utils.batched_weighted_sum(
            best_rewards[:, 0] - best_rewards[:, 1],
            (self.deltas.get(d_idx, self.w_policy.size) for d_idx in best_deltas_idx),
            batch_size=500
        )

        # 10. Final Scaling
        # We divide by the number of used directions and the reward standard deviation
        return g_hat / (len(best_deltas_idx) * sigma_r)
    ...
    def train(self, total_timesteps=3000000):
        ...
        try:
            while self.timesteps < total_timesteps:
                ...
                # Use -= because we want to follow the direction of the gradient step
                self.w_policy -= self.optimizer._compute_step(g_hat).reshape(self.w_policy.shape)

                # 4. Housekeeping: Distributed Filter Sync (The "V2" Logic)
                all_worker_filters = ray.get([worker.get_filter.remote() for worker in self.workers])

                for w_filter in all_worker_filters:
                    # Update master filter with the BUFFER (new data) from workers
                    self.policy.observation_filter.update(w_filter)

                # Recompute mean/std from accumulated stats
                self.policy.observation_filter.stats_increment()
                # Clear master buffer so we don't double-aggregate next iteration
                self.policy.observation_filter.clear_buffer()

                # Sync the fresh stats back to the workers
                filter_id = ray.put(self.policy.observation_filter)
                ray.get([worker.sync_filter.remote(filter_id) for worker in self.workers])

                # Critical: Workers must now update their internal local variables and clear buffers
                ray.get([worker.stats_increment.remote() for worker in self.workers])
                ray.get([worker.clear_buffer.remote() for worker in self.workers])

                # 5. Save Weights (Every 10 iterations)
                if iteration % 10 == 0:
                ...
                # 6. LOGGING
                if (iteration % 10 == 0):
                    # Eval rollouts don't update the filter (policy.update_filter=False)
            ...
            # Ensure we get the latest weights and the finalized stats
            w = ray.get(self.workers[0].get_weights_plus_stats.remote())
            np.savez(self.logdir + "/lin_policy_plus", **w)
            self.writer.close()
