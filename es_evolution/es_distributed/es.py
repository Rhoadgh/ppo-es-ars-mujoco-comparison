...
class SharedNoiseTable(object):
    def __init__(self, seed=123):  # 1. Add argument here
        import ctypes, multiprocessing
        # seed = 123  <--- DELETE THIS
        count = 250000000
...
def run_master(master_redis_cfg, log_dir, exp):
    ...
    current_seed = exp.get('seed', 123)
    noise = SharedNoiseTable(seed=current_seed)

    rs = np.random.RandomState(current_seed)
    ob_stat = RunningStat(
    ...
    while True:
        ...
        writer.add_scalar('Reward/Mean', returns_n2.mean(), timesteps_so_far)
        writer.add_scalar('Reward/Eval', np.nan if not eval_rets else np.mean(eval_rets), timesteps_so_far)
        writer.add_scalar('Steps/Iteration', lengths_n2.sum(), timesteps_so_far)
        writer.flush()
